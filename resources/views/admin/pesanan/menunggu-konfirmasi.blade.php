            ...
            <div class="modal-content">
                <div class="modal-body">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"
                        style="margin-top: -10px"><span aria-hidden="true">&times;</span></button>
...
@section('scripts')
    <script>
        function konfirmasi(id) {
            if ($(`#ongkir-${id}`).val().length == 0) {
                alert('Masukkan ongkir')
            } else {
                let ongkir = parseInt($(`#ongkir-${id}`).val().replaceAll('.', ''))
                $.ajax({
                    type: 'post',
        ...
        // $('.input-ongkir').on('input', function() {
