            ...
            <tbody>
                @foreach (Auth::user()->keranjang()->where('type', 'reguler')->get() as $k)
                    <tr>
                        ...
                        <td>
                            <a href="{{ $k->url }}"><img src="{{ asset('user/barang_img/' . $k->gambar) }}"
                                    class="keranjang-gambar-produk"></a>
                        ...
                        <td>
                            <div class="quantity">
                                <div class="quantity-select">
                                    ...
                                    <div id="{{ $k->id }}" class="entry value"><span>{{ $k->jumlah }}</span>
                                    </div>
                                    <div data-idkeranjang="{{ $k->id }}" class="entry value-plus active">&nbsp;
                                    </div>
                                </div>
                            </div>
                        </td>
        ...
        <div class="checkout-left-basket" data-wow-delay=".5s">
            ...
            <ul>
                <li>Total <i class="far fa-info-circle" style="margin-left: 5px" data-toggle="tooltip"
                        data-placement="right" title="Total harga sudah termasuk ongkos kirim"></i>
                    <span>Rp {{ number_format($total) }}</span>
                </li>
                <li class="lanjut-bayar">
                    <h5>Lanjut Pesanan <i class="fas fa-chevron-right" style="margin-left: 10px"></i></h5>
                </li>
            </ul>
        </div>
